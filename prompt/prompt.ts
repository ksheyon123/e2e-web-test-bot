...
4. 좌표 측정 절차
  4-1. 그리드 포인트 정의:
    - 유효한 그리드 포인트: 수직선과 수평선이 실제로 교차하고, 해당 좌표가 상단과 좌측에 숫자로 표시된 지점
    - 그리드 라인만 있고 숫자가 없는 지점은 사용 불가
    - 숫자를 추측하거나 보간하여 계산된 좌표는 사용 불가
    - 표시되지 않은 중간 좌표는 사용 불가

  4-2. 교차점과 내부 정의:
    - 교차점: 수직 그리드 선과 수평 그리드 선이 만나는 지점
    - 내부: 교차점이 컴포넌트의 테두리 안쪽에 있는 경우
    - 테두리 위: 교차점이 테두리 선 위에 있는 경우도 내부로 간주
    - 외부: 교차점이 컴포넌트의 테두리 바깥에 있는 경우

  4-3. 그리드가 있는 이미지의 경우:
    a. X 좌표 측정:
      - 컴포넌트의 테두리 좌상단에서 시작하여 왼쪽으로 가장 가까운 유효한 그리드 포인트 찾기
      - 해당 교차점이 컴포넌트 내부에 있는지 확인
      - 내부에 있고 상단에 숫자가 표시된 경우만 해당 값을 X 좌표로 사용
      - 조건을 만족하지 않으면 다음 유효한 그리드 포인트로 이동하여 반복

    b. Y 좌표 측정:
      - 컴포넌트의 테두리 좌상단에서 시작하여 위쪽으로 가장 가까운 유효한 그리드 포인트 찾기
      - 해당 교차점이 컴포넌트 내부에 있는지 확인
      - 내부에 있고 좌측에 숫자가 표시된 경우만 해당 값을 Y 좌표로 사용
      - 조건을 만족하지 않으면 다음 유효한 그리드 포인트로 이동하여 반복
